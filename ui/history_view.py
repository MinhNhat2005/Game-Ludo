...
from utils.database_manager import get_match_history, get_action_history
...
class HistoryView:
    def __init__(self, screen, manager, sound_manager):
        ...
        self.selected_match_id = None # Sẽ lưu ID trận đấu được chọn
        ...
        if not match_list:
            ...
            for match in match_list:
                ...
                # --- SỬA LOGIC HIỂN THỊ ---
                # Chỉ hiển thị các game "Đang chơi dở" (is_loadable = True)
                if is_loadable:
                    status_text = "(Đang chơi dở - Click để tiếp tục)"
                # Hoặc các game đã kết thúc CÓ NGƯỜI THẮNG
                elif match['WinnerPlayerID'] is not None:
                    status_text = f"(Người {match['WinnerPlayerID'] + 1} thắng)"
                # Bỏ qua các game "Đã kết thúc - Hòa?" (trừ khi bạn muốn xem)
                else:
            ...
            if event.ui_element == self.history_list:
                ...
                if selected_item_id and selected_item_id in self.match_data_map:
                    ...
                    if match_info.get('is_loadable', False):
                        ...
                        self.is_running = False # 1. Dừng view này
                        self.next_screen = 'load_game' # 2. Gửi tín hiệu 'load_game'
    ...
    def update(self, time_delta):
        self.manager.update(time_delta)# ui/history_view.py
import pygame
import pygame_gui
import logging
from utils.constants import WIDTH, HEIGHT
...
class HistoryView:
    def __init__(self, screen, manager, sound_manager):
        ...
        # Lấy dữ liệu từ CSDL
        # get_match_history() giờ đã được cập nhật để trả về các game "is_loadable"
        match_list = get_match_history()

        item_list_data = []
        self.match_data_map = {} # Dùng để tra cứu thông tin khi click

        if not match_list:
            item_list_data.append( ("Chưa có lịch sử trận đấu nào.", "no_history") )
        else:
            for match in match_list:
                # Kiểm tra xem có phải là game đang chơi dở không
                is_loadable = match.get('is_loadable', False)
                
                # Phân biệt game dở và game đã kết thúc
                if is_loadable:
                    status_text = "(Đang chơi dở - Click để tiếp tục)"
                elif match['WinnerPlayerID'] is not None:
                    status_text = f"(Người {match['WinnerPlayerID'] + 1} thắng)"
                else:
                    status_text = "(Đã kết thúc - Không rõ)" # Phòng trường hợp game bị bỏ dở
                ...
                item_list_data.append( (display_text, match_id_str) )
                # Lưu lại thông tin của trận này
                self.match_data_map[match_id_str] = match 
            ...
            if event.ui_element == self.history_list:
                ...
                if selected_item_id and selected_item_id in self.match_data_map:
                    ...
                    if match_info.get('is_loadable', False):
                        ...
                        # Chỉ xem chi tiết hành động (nếu muốn)
                        logging.info(f"HistoryView: Xem lịch sử trận đã kết thúc: MatchID {match_info['MatchID']}")
                        actions = get_action_history(match_info['MatchID'])
                        logging.info(f"--- Chi tiết trận {match_info['MatchID']} ---")
                        for action in actions:
